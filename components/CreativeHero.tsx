...
function VerticalColumn({ items, speed, reverse = false }: { items: any[], speed: number, reverse?: boolean }) {
    ...
    <div className="flex-1 min-w-37.5 md:min-w-50 lg:min-w-60 relative h-full hero-column will-change-transform">
      <div ref={columnRef} className="flex flex-col gap-6 py-4">
        {displayItems.map((item, i) => (
          <div key={i} className="relative aspect-4/5 w-full bg-[#0a0a0a] overflow-hidden rounded-2xl border border-white/5 shadow-2xl">
...
export default function CreativeHero({ isReady = false, mediaItems }: { isReady?: boolean; mediaItems: readonly MediaItem[] }) {
  ...
  useEffect(() => {
    ...
    if (!isReady || !titleRef.current || !subRef.current || !containerRef.current) return;
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(titleSplit.chars, {
        ...
        .to(subRef.current, {
          ...
          duration: 0.5,
          ease: "power3.out",
        }, "+=0.2") // Inicia 0.2s DESPUÉS de que el título termine
        .to(".hero-column-group", {
          opacity: 1,
          y: 0,
          duration: 1.2,
          ease: "power2.out",
        }, "-=0.6") // Los videos entran suavemente mientras se termina de leer
        .to(".scroll-indicator", {
          opacity: 0.6,
          duration: 0.6,
        }, "-=0.3");
  ...
  return (
    <section ref={containerRef} className="relative h-screen w-full bg-[#050505] overflow-hidden flex items-center justify-center">
      ...
      {/* Background Marquee Columns - Sin 'invisible', controlado por opacity: 0 inicial en JS */}
      <div className="absolute inset-0 flex gap-4 p-4 hero-column-group opacity-0">
      ...
      <div className="absolute inset-0 z-10 pointer-events-none" style={{ background: 'radial-gradient(circle 800px at center, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.95) 100%)' }} />

      {/* Hero Content */}
      <div ref={contentRef} className="relative z-20 text-center px-10 max-w-5xl">
        <h1
          ref={titleRef}
